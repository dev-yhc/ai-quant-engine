...
func (r *Repository) MarkSignalDelivered(ctx context.Context, id int64) error {
	_, err := r.pool.Exec(ctx, `UPDATE valuation.signal_outbox SET status = 'DELIVERED', processed_at = NOW(), locked_at = NULL WHERE id = $1`, id)
	return err
}

func (r *Repository) MarkSignalRetry(ctx context.Context, id int64, cause error) error {
	_, err := r.pool.Exec(ctx, `UPDATE valuation.signal_outbox SET status = 'PENDING', available_at = NOW() + INTERVAL '1 minute', locked_at = NULL, last_error = $2 WHERE id = $1`, id, cause.Error())
	return err
}
...
func (r *Repository) MarkAlertDelivered(ctx context.Context, id int64) error {
	_, err := r.pool.Exec(ctx, `UPDATE valuation.alert_outbox SET status = 'DELIVERED', delivered_at = NOW(), locked_at = NULL WHERE id = $1`, id)
	return err
}

func (r *Repository) MarkAlertRetry(ctx context.Context, id int64, cause error) error {
	_, err := r.pool.Exec(ctx, `UPDATE valuation.alert_outbox SET status = 'PENDING', available_at = NOW() + INTERVAL '1 minute', locked_at = NULL, last_error = $2 WHERE id = $1`, id, cause.Error())
	return err
}
